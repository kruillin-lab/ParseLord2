...
using ParseLord2.Combos.PvE.Enums;
...
using ParseLord2.Services.IPC_Subscriber;
...
using ParseLord2.AutoRotation.Engine;
...
{
    public static AutoRotationConfig? cfg;
    ...
    private const int PendingDpsRetryWindowMs = 450;
    private const int PendingDpsRetryIntervalMs = 50;

    private static PendingAction? _pendingDps;
    ...
    private static DateTime? ManualOverrideUntil;
    ...
    private static bool TryExecutePendingDps()
    {
        if (_pendingDps is null)
            return false;

        var now = Environment.TickCount64;
        var p = _pendingDps.Value;

        // Expired or not valid anymore
        if (now >= p.ExpiresAtMs || p.IsGroundTarget)
        {
            _pendingDps = null;
            return false;
        }

        // If we've failed multiple times, stop retrying this action to avoid starving other oGCDs.
        if (p.Attempts >= 4)
        {
            _pendingDps = null;
            return false;
        ...
        // If action is no longer ready, abandon retries.
        if (!ActionReady(p.ActionId))
        {
            _pendingDps = null;
            return false;
        }

        // Throttle spam attempts a little to avoid hammering the client every tick
        if (now < p.NextAttemptAtMs)
            return true;

        Service.ActionReplacer.DisableActionReplacingIfRequired();
        WouldLikeToGroundTarget = ActionSheet[p.ActionId].TargetArea;
        var ret = ActionManager.Instance()->UseAction(ActionType.Action, p.ActionId, p.TargetId);
        ...
        {
            _pendingDps = null;
            // Preserve existing AoE lock behavior for NIN Mudra signs if applicable
            if (NIN.MudraSigns.Contains(p.ActionId))
                _lockedAoE = true;
            else
                _lockedAoE = false;
        ...
        p.Attempts++;
        p.NextAttemptAtMs = now + PendingDpsRetryIntervalMs;
        _pendingDps = p;
        return true;
    }
    ...
    {
        cfg ??= Service.Configuration.RotationConfig;
        ...
        // Healer logic (RSR-style thresholds + healer override phase input)
        bool isHealer = Player.Object?.Role is CombatRole.Healer;

        // Party scan for thresholds (party only; optional out-of-party NPCs controlled by setting)
        var partyMembers = GetPartyMembers();
        if (!cfg.HealerSettings.IncludeNPCs)
            partyMembers.RemoveAll(x => x.IsOutOfPartyNPC);

        // NOTE: RSR-style triggers are based on party HP%, not on whether a heal action is already selected.
        bool anyTankBelow = false;
        bool anyMemberBelow = false;
        bool anyEmergency = false;
        int injuredCount = 0;

        

        int playerLevel = LocalPlayer?.Level ?? 0;

        foreach (var pm in partyMembers)
        {
            var bc = pm.BattleChara;
            if (bc == null) continue;
            if (bc.MaxHp <= 0) continue;

            var hpPct = (int)((bc.CurrentHp * 100f) / bc.MaxHp);

            bool hasHot = JobID switch
            {
                Job.AST => HasStatusEffect(AST.Buffs.AspectedBenefic, bc)
                        || HasStatusEffect(AST.Buffs.AspectedHelios, bc)
                        || HasStatusEffect(AST.Buffs.HeliosConjunction, bc),

                Job.WHM => HasStatusEffect(WHM.Buffs.Regen, bc)
                        || HasStatusEffect(WHM.Buffs.Medica2, bc)
                        || HasStatusEffect(WHM.Buffs.Medica3, bc)
                        // Asylum (regen field)
                        || HasStatusEffect(1911, bc)
                        || HasStatusEffect(1912, bc),

                Job.SCH => HasStatusEffect(315, bc)            // Whispering Dawn
                        || HasStatusEffect(1222, bc)          // Fey Union
                        || HasStatusEffect(1223, bc)          // Fey Union (variant)
                        || HasStatusEffect(1874, bc)          // Angel's Whisper
                        || HasStatusEffect(3885, bc)          // Seraphism (variant)
                        || HasStatusEffect(4327, bc)          // Seraphism (variant)
                        || HasStatusEffect(1944, bc)          // Sacred Soil (regen)
                        || (playerLevel >= 78 && (HasStatusEffect(298, bc)
                                              || HasStatusEffect(299, bc)
                                              || HasStatusEffect(2637, bc)
                                              || HasStatusEffect(2638, bc))),

                Job.SGE => HasStatusEffect(2605, bc)           // Kardion
                        || HasStatusEffect(2872, bc)          // Kardion (variant)
                        || HasStatusEffect(2617, bc)          // Physis
                        || HasStatusEffect(2620, bc)          // Physis II
                        || HasStatusEffect(3898, bc)          // Philosophia
                        || HasStatusEffect(2938, bc)          // Kerakeia
                        || (playerLevel >= 78 && HasStatusEffect(2618, bc)),

                _ => false,
            };
            int partyThreshold = hasHot ? cfg.HealerSettings.PartyHealHPPWithRegen : cfg.HealerSettings.PartyHealHPP;
            int tankThreshold = hasHot ? cfg.HealerSettings.TankHealHPPWithRegen : cfg.HealerSettings.TankHealHPP;

            if (hpPct <= partyThreshold)
                injuredCount++;

            if (hpPct <= cfg.HealerSettings.EmergencyHealHPP)
                anyEmergency = true;

            if (hpPct <= partyThreshold)
                anyMemberBelow = true;

            // Trust-friendly tank detection using real job row id (PLD/WAR/DRK/GNB).
            var jobRowId = pm.RealJob?.RowId ?? 0;
            bool isTank = jobRowId is 19 or 21 or 32 or 37;
            if (isTank && hpPct <= tankThreshold)
                anyTankBelow = true;
        }

        bool preferAoeHeal = injuredCount >= cfg.HealerSettings.AoEHealMinInjuredCount;

        var healTarget = isHealer ? AutoRotationHelper.GetSingleTarget(cfg.HealerRotationMode) : null;

        bool hasAnyHealAction = autoActions.Any(x => x.Key.Attributes()?.AutoAction?.IsHeal == true);
        bool hasAnyAoeHealAction = autoActions.Any(x => x.Key.Attributes()?.AutoAction?.IsHeal == true && x.Key.Attributes()?.AutoAction?.IsAoE == true);
        ...
        bool needsHeal = isHealer
                         && hasAnyHealAction
                         && (anyEmergency || anyTankBelow || anyMemberBelow || aoeheal || healTarget != null);
        ...
        // RSR-style phase model: compute rez/cleanse eligibility here, then let the engine schedule tasks.
        bool inOpener = WrathOpener.CurrentOpener?.CurrentState is OpenerState.InOpener;
        bool canRez = !inOpener && (
            isHealer ||
        ...
        // Cleanse is lower priority than rez (and suppressed during emergency healing).
        if (cfg.HealerSettings.AutoCleanse && isHealer && !anyEmergency && !inOpener)
            CleanseParty();
        // SGE Kardia logic
        ...
        // Manual input override: if you're pressing keys/mouse, autorotation backs off for a configurable window.
        if (cfg.ManualInputOverrideMs > 0)
        {
            if (ManualInputDetector.IsManualInputActive())
                ManualOverrideUntil = DateTime.Now.AddMilliseconds(cfg.ManualInputOverrideMs);

            if (ManualOverrideUntil is not null && DateTime.Now < ManualOverrideUntil.Value)
                return;
        }

        // RSR-style engine tick: tasks are scheduled by priority (Emergency Heal > Rez > Heal > DPS).
        ...
        AutoRotationEngine.Tick(ctx);
        _ = ctx.Temp;
    }

    internal static bool ProcessAutoActions(Dictionary<Preset, bool> autoActions, ref uint _, bool canHeal, bool stOnly)
    {
        // DPS execution retry: if an oGCD was selected but failed due to animation lock / timing,
        // keep retrying it briefly (Advanced-mode-style) so Gold planners don't lose throughput.
        if (!canHeal && TryExecutePendingDps())
            return true;
        ...
        foreach (var entry in filteredActions)
        {
            ...
            // Use adjusted action id so autorotation still works when the base action is trait-upgraded
            // (e.g., True Thrust -> Raiden Thrust). The existing ReplaceSkill attribute remains the
            // stable "anchor" for combo routing, but the engine must validate/queue the actual
            // executable action.
            uint gameActBase = attributes.ReplaceSkill!.ActionIDs.First();
            uint gameAct = ActionManager.Instance()->GetAdjustedActionId(gameActBase);
    ...
    internal static bool RezParty()
    {
        ...
        if (Player.Object.CurrentMp >= GetResourceCost(resSpell) && ActionReady(resSpell))
        {
            ...
            var rezTarget = deadPeople
                .Where(RezQuery)
                .OrderBy(x => x.GetRole() switch
                {
                    CombatRole.Healer => 0,
                    CombatRole.Tank => 1,
                    CombatRole.DPS => 2,
                ...
                .ThenBy(x => GetTargetDistance(x.BattleChara, Player.Object))
                .FirstOrDefault();

            if (rezTarget is null)
                return false;

            var member = rezTarget;
    ...
    public static class AutoRotationHelper
    {
        ...
        public static bool ExecuteAoE(Enum mode, Preset preset, Presets.PresetAttributes attributes, uint gameAct)
        {
            ...
            if (attributes.AutoAction!.IsHeal)
            {
                ...
                // RSR-style per-action enable/disable override (defaults to enabled if missing)

                if (Service.Configuration.ActionEnabled.TryGetValue(outAct, out var enabled) && !enabled)
...
if (outAct is All.SavageBlade) return true;
        ...
        public static bool ExecuteST(Enum mode, Preset preset, Presets.PresetAttributes attributes, uint gameAct)
        {
            ...
            if (canUse && (inRange || areaTargeted))
            {
                ...
                else if (!isHeal && !ret && ActionSheet[outAct].ActionCategory.RowId == (uint)ActionAttackType.Ability && !ActionSheet[outAct].TargetArea)
                {
                    // Retry briefly (for animation/queue timing), but cap attempts to avoid starving other oGCDs.
                    var now = Environment.TickCount64;
                    _pendingDps = new PendingAction
                    {
                        ActionId = outAct,
                        TargetId = targetId,
                        ExpiresAtMs = now + PendingDpsRetryWindowMs,
                        NextAttemptAtMs = now + PendingDpsRetryIntervalMs,
    ...
    public static class HealerTargeting
    {
        ...
        {
            return TargetHasRegen((IGameObject?)target);
        }
