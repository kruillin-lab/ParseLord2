...
internal partial class AST : Healer
{
    ...
    internal class AST_ST_Heals : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            // Tank HoT maintenance: if enabled and no explicit target was provided, prioritize a party tank missing Aspected Benefic below the configured threshold.
            if (OptionalTarget is null)
            {
                var tankNeedingAspectedBenefic = GetPartyTankNeedingAspectedBenefic();
                if (tankNeedingAspectedBenefic is not null)
                    healTarget = tankNeedingAspectedBenefic;
            }
