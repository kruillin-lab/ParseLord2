...
using static ParseLord2.Combos.PvE.AST.Config;
...
internal partial class AST
{
    ...
    /// <summary>
    /// Finds a party tank (not the player) that is below the configured HP% threshold and is missing Aspected Benefic.
    /// Party-only (not alliance).
    /// </summary>
    internal static IGameObject? GetPartyTankNeedingAspectedBenefic()
    {
        if (!AST_ST_SimpleHeals_AspectedBeneficKeepOnTank)
            return null;

        var threshold = AST_ST_SimpleHeals_AspectedBeneficKeepOnTankHPThreshold;

        IGameObject? best = null;
        var bestHp = 101f;

        foreach (var pm in SimpleTarget.GetPartyMembers())
        {
            var bc = pm.BattleChara;
            if (bc is null || bc.IsDead || !bc.IsNotThePlayer())
                continue;

            // Trust-safe tank detection: use party member real job when available.
            var job = pm.RealJob?.GetJob();
            if (job is not Job.PLD and not Job.WAR and not Job.DRK and not Job.GNB )
                continue;

            var hp = GetTargetHPPercent(bc, AST_ST_SimpleHeals_IncludeShields);
            if (hp > threshold)
                continue;

            var hot = GetStatusEffect(Buffs.AspectedBenefic, bc);
            if (hot is not null)
                continue; // missing-only behavior
    ...
    internal static int GetMatchingConfigST(int i, IGameObject? OptionalTarget, out uint action, out bool enabled)
    {
        ...
        switch (i)
        {
            ...
            case 7:
                ...
                // Optional: keep Aspected Benefic rolling on party tanks while they are below the configured HP%.
                // For the "keep on tank" feature, we use missing-only behavior (no refresh timer).
                bool keepOnTankActive = AST_ST_SimpleHeals_AspectedBeneficKeepOnTank &&
                                        tankCheck &&
                                        GetTargetHPPercent(healTarget, AST_ST_SimpleHeals_IncludeShields) <= AST_ST_SimpleHeals_AspectedBeneficKeepOnTankHPThreshold;

                bool needsRefresh = keepOnTankActive
                ...
                enabled = IsEnabled(Preset.AST_ST_Heals_AspectedBenefic) &&
                          ...
                          needsRefresh &&
                          (keepOnTankActive || !stopHot);

                return keepOnTankActive ? AST_ST_SimpleHeals_AspectedBeneficKeepOnTankHPThreshold : AST_ST_SimpleHeals_AspectedBeneficHigh;
            case 8:
