...
using static ParseLord2.Combos.PvE.DNC.Config;
...
internal partial class DNC
{
    ...
    /// </returns>
    private static bool InAutoMode(bool singleTarget) =>
        P.IPC.GetAutoRotationState() &&
        (
            P.IPC.GetComboState(
                (singleTarget ? Preset.DNC_ST_AdvancedMode : Preset.DNC_AoE_AdvancedMode).ToString()
            )?.Values.Last() ?? false
        );
