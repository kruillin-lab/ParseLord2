...
internal partial class WHM : Healer
{
    ...
    internal class WHM_SimpleST_Heals : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            // Tank HoT maintenance: if enabled and no explicit target was provided, prioritize a party tank missing Regen below the configured threshold.
            if (OptionalTarget is null)
            {
                var tankNeedingRegen = GetPartyTankNeedingRegen();
                if (tankNeedingRegen is not null)
                    healTarget = tankNeedingRegen;
            }
