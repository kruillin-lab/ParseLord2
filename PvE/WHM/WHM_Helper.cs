...
using static ParseLord2.Combos.PvE.WHM.Config;
...
internal partial class WHM
{
    ...
    /// <summary>
    /// Finds a party tank (not the player) that is below the configured HP% threshold and is missing Regen.
    /// Party-only (not alliance).
    /// </summary>
    internal static IGameObject? GetPartyTankNeedingRegen()
    {
        if (!WHM_STHeals_RegenKeepOnTank)
            return null;

        var threshold = WHM_STHeals_RegenKeepOnTankHPThreshold;
        ...
        foreach (var pm in SimpleTarget.GetPartyMembers())
        {
            ...
            var hp = GetTargetHPPercent(bc, WHM_STHeals_IncludeShields);
            if (hp > threshold)
                continue;

            var regen = GetStatusEffect(Buffs.Regen, bc);
        ...
        out uint action, out bool enabled)
    {
        ...
        switch (i)
        {
            ...
            case 5:
                ...
                // Optional: keep Regen rolling on party tanks while they are below the configured HP%.
                // For the "keep on tank" feature, we use missing-only behavior (no refresh timer).
                bool keepOnTankActive = WHM_STHeals_RegenKeepOnTank &&
                                        tankCheck &&
                                        GetTargetHPPercent(healTarget, WHM_STHeals_IncludeShields) <= WHM_STHeals_RegenKeepOnTankHPThreshold;
                ...
                return keepOnTankActive ? WHM_STHeals_RegenKeepOnTankHPThreshold : WHM_STHeals_RegenHPUpper;
