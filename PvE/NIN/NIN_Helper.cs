...
internal static bool NinjaWeave => CanWeave(.6f, 10);
    ...
    internal static float DotonTimeStill => NIN_AoE_AdvancedMode_Ninjitsus_Doton_TimeStill;
    ...
    internal static bool CanUseRaiton => LevelChecked(Raiton) && MudraReady &&
                                          (!HasKassatsu || IsNotEnabled(Preset.NIN_ST_AdvancedMode_Ninjitsus_Hyosho) || !LevelChecked(HyoshoRanryu)) && //Use kassatsu on it if Hyosho isn't selected. 
                                           ...
                                           !NIN_ST_AdvancedMode_Ninjitsus_Raiton_Pooling || //Dont Pool because of Raiton Option
    ...
    internal static bool CanUseKaton => LevelChecked(Katon) && MudraReady &&
                                         (!HasKassatsu || IsNotEnabled(Preset.NIN_AoE_AdvancedMode_Ninjitsus_Goka)) &&
                                         (TrickDebuff || //Buff Window
                                          !LevelChecked(Huton) || //Dont Pool because of Huton not learned yet
                                          GetCooldownChargeRemainingTime(Ten) < 1 || // Spend to avoid cap
                                          !NIN_AoE_AdvancedMode_Ninjitsus_Katon_Pooling || //Dont Pool
    ...
    internal static bool CanUseHyoshoRanryu => LevelChecked(HyoshoRanryu) && MudraReady && HasKassatsu &&
                                               (BuffWindow || IsNotEnabled(Preset.NIN_ST_AdvancedMode_TrickAttack) || KassatsuRemaining < 3);
    ...
    internal static bool TrickDisabledST => IsNotEnabled(Preset.NIN_ST_AdvancedMode_TrickAttack);
    internal static bool TrickDisabledAoE => IsNotEnabled(Preset.NIN_AoE_AdvancedMode_TrickAttack);
    internal static bool MugDisabledST => IsNotEnabled(Preset.NIN_ST_AdvancedMode_Mug);
    internal static bool MugDisabledAoE => IsNotEnabled(Preset.NIN_AoE_AdvancedMode_Mug);
