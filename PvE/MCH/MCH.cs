...
using static ParseLord2.Combos.PvE.MCH.Config;
...
internal partial class MCH : PhysicalRanged
{
    internal class MCH_ST_SimpleMode : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            // All weaves
            // Advanced mode should be tolerant of real-world weave windows.
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            if (!IsOverheated)
            {
                ...
                if (ActionReady(Flamethrower) &&
                    ...
                    // Patch 7.4 note (Icy-Veins): Bioblaster is a gain starting at 3 targets.
                    // Keep this consistent with Advanced ST/AoE tuning to avoid drifting tool usage.
                    if (ActionReady(BioBlaster) &&
                        NumberOfEnemiesInRange(Scattergun, CurrentTarget) >= 3 &&
    ...
    internal class MCH_ST_AdvancedMode : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            if (CanWeave() || CanDelayedWeave())
            {
                ...
                {
                    // Reassemble
                    // Meta: don't drift Reassemble and don't waste it on filler.
                    // Prefer using Reassemble only when a tool hit is imminent.
                    if (IsEnabled(Preset.MCH_ST_Adv_Reassemble) &&
                        GetRemainingCharges(Reassemble) > MCH_ST_ReassemblePool &&
                        GetTargetHPPercent() > HPThresholdReassemble &&
                        CanReassemble() &&
                        (ActionReady(Drill) || ActionReady(AirAnchor) || ActionReady(Chainsaw) ||
                         (LevelChecked(Excavator) && HasStatusEffect(Buffs.ExcavatorReady)) ||
                         GetCooldownRemainingTime(Drill) <= GCD * 1.25f ||
                         GetCooldownRemainingTime(AirAnchor) <= GCD * 1.25f ||
                         GetCooldownRemainingTime(Chainsaw) <= GCD * 1.25f))
                        return Reassemble;

                    // Barrel Stabilizer
                    // Meta: don't over-gate this behind "all tools on cooldown".
                    // Primary intent is to enable Hypercharge/Wildfire pacing and prevent Heat stagnation.
                    if (IsEnabled(Preset.MCH_ST_Adv_Stabilizer) &&
                        ...
                        !IsOverheated &&
                        Heat <= 50 &&
                        (ActionReady(Wildfire) || GetCooldownRemainingTime(Wildfire) <= GCD * 2))
                        return BarrelStabilizer;
    ...
    internal class MCH_AoE_AdvancedMode : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            if (!IsOverheated)
            {
                ...
                {
                    // 7.4+: Bioblaster is generally a 3+ target tool.
                    if (ActionReady(BioBlaster) &&
                        NumberOfEnemiesInRange(Scattergun, CurrentTarget) >= 3 &&
                        !HasStatusEffect(Debuffs.Bioblaster, CurrentTarget) &&
                        !HasStatusEffect(Buffs.Reassembled) &&
