...
public static class IconButtons
{
    ...
    public static bool IconImageButton(string imageUrl, string text, Vector2 size = new(), bool imageOnRight = false, float imageScale = 0)
    {
        ...
        if (TryGetLocalTextureWrap(imageUrl, out var texture))
        {
    ...
    private static bool TryGetLocalTextureWrap(string pathOrUrl, out IDalamudTextureWrap? wrap)
    {
        wrap = null;
        ...
        try
        {
            var tex = Svc.Texture.GetFromFile(pathOrUrl);
            if (tex.TryGetWrap(out var w, out _))
            {
                wrap = w;
                return true;
            }
