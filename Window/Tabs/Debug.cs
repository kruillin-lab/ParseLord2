    ...
    internal new static unsafe void Draw()
    {
        ...
        if (ImGui.CollapsingHeader("Wrath IPC"))
        {
            ...
            if (_wrathLease is null)
            {
                ...
                {
                    _wrathLease = P.IPC.RegisterForLease("ParseLord2", "ParseLord2", WrathIPCCallback);
                }
                ImGui.Unindent();
            }
            ...
            if (_wrathLease is not null)
            {
                ...
                {
                    // https://git.carvel.li/liza/Questionable/src/commit/de90882ecbb609c2f79fecc1ec17b751dc8763f2/Questionable/Controller/CombatModules/ParseLord2Module.cs#L68
                    P.IPC.SetAutoRotationState(_wrathLease!.Value);
                    P.IPC.SetCurrentJobAutoRotationReady(_wrathLease!.Value);
                }
            }
        ...
        if (ImGui.CollapsingHeader("Hybrid Planner Trace (Option 3)"))
        {
            var trace = PlannerTraceStore.GetLatest();
            if (trace == null)
            {
                ImGui.TextDisabled("No planner trace captured yet. Enable a Gold Planner preset and press a few GCDs.");
            }
            else
            {
                ImGui.Text($"Job: {trace.JobLabel}");
                ImGui.Text($"Timestamp (UTC): {trace.TimestampUtc:O}");

                ImGui.Separator();

                ImGui.Text($"Burst Phase: {trace.BurstPhase}");
                ImGui.Text($"Target Count (est): {trace.TargetCountEstimate}");
                var weaveLegalText = trace.IsInLegalWeaveWindow ? "Legal" : "Not Legal";
                ImGui.Text($"Weave Window: {weaveLegalText}  Slots: {trace.WeaveSlotsUsed}/{trace.WeaveSlotsMax}");

                ImGui.Separator();

                var plannedGcdId = trace.PlannedNextGcdActionId;
                ImGui.Text($"Planned Next GCD: {GetActionName(plannedGcdId)} (ID: {plannedGcdId})");
                var selectedOgcdName = trace.SelectedOgcdActionId != 0 ? GetActionName(trace.SelectedOgcdActionId) : "None";
                ImGui.Text($"Selected oGCD: {selectedOgcdName} (ID: {trace.SelectedOgcdActionId})");

                ImGuiEx.Spacing(new Vector2(0, SpacingSmall));

                var candidates = trace.OgcdCandidates;
                if (candidates == null || candidates.Count == 0)
                {
                    ImGui.TextDisabled("No oGCD candidates recorded for the last decision.");
                }
                else
                {
                    var sorted = candidates
                        .OrderByDescending(c => c.Allowed)
                        .ThenByDescending(c => c.Score)
                        .Take(14)
                        .ToList();

                    using var table = ImRaii.Table("##HybridPlannerTraceCandidates", 5,
                    ...
                    if (table)
                    {
                        ...
                        ImGui.TableSetupColumn("Block Reason", ImGuiTableColumnFlags.WidthStretch);
                        ...
                        foreach (var c in sorted)
                        {
                            ...
                            ImGui.TextUnformatted(GetActionName((uint)c.ActionId));

                            ImGui.TableNextColumn();
                            ImGui.TextUnformatted(c.ActionId.ToString());

                            ImGui.TableNextColumn();
                            ImGui.TextUnformatted(c.Allowed ? "Yes" : "No");

                            ImGui.TableNextColumn();
                            ImGui.TextUnformatted(c.Allowed ? c.Score.ToString("N1") : "-");

                            ImGui.TableNextColumn();
                            ImGui.TextUnformatted(c.Allowed ? "" : c.BlockReason);
                        }
                    }
                }
            }
