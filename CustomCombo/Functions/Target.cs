...
internal abstract partial class CustomComboFunctions
{
    ...
    /// <summary>
    /// Checks if a target is within the player's vision cone.
    /// Vision cone is centered on the player's facing direction.
    /// </summary>
    /// <param name="optionalTarget">Target to check. Defaults to CurrentTarget.</param>
    /// <param name="coneAngleDegrees">Cone angle in degrees (0-360). 180 = half-circle, 360 = full circle.</param>
    /// <returns>True if target is within the vision cone.</returns>
    public static bool IsTargetInVisionCone(IGameObject? optionalTarget = null, float coneAngleDegrees = 120f)
    {
        ...
        // If cone is 360 degrees or more, everything is in the cone
        if (coneAngleDegrees >= 360f)
            return true;

        // Calculate angle from player facing direction to target
        // Player rotation is where they're facing
        // We need the angle from player to target
        float rotation = PositionalMath.GetRotation(player.Position, target.Position) - player.Rotation;
        float angleDegrees = PositionalMath.ToDegrees(rotation);
        
        // Normalize to 0-360 range
        if (angleDegrees < 0f)
            angleDegrees += 360f;
        
        // Convert to -180 to +180 range for easier cone check
        if (angleDegrees > 180f)
            angleDegrees -= 360f;

        // Check if target is within cone angle (centered at 0 degrees = straight ahead)
