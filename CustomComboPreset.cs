...
public enum Preset
{
    ...
    [ReplaceSkill(BRD.HeavyShot, BRD.BurstShot)]
    [CustomComboInfo("Gold Planner - Single Target",
        "Option 3 (Hybrid Planner): deterministic BRD GCD planning + generic score-based oGCD selection with full trace output.",
        Job.BRD)]
    [AdvancedCombo]
    BRD_ST_GoldPlanner = 3063,

    [ConflictingCombos(BRD_AoE_AdvMode, BRD_AoE_SimpleMode)]
    [ReplaceSkill(BRD.QuickNock, BRD.Ladonsbite)]
    [CustomComboInfo("Gold Planner - AoE",
        "Option 3 (Hybrid Planner): deterministic BRD AoE GCD planning + generic score-based oGCD selection with full trace output.",
        Job.BRD)]
    [AdvancedCombo]
    BRD_AoE_GoldPlanner = 3064,
    ...
    [AutoAction(false, false)]
    [ReplaceSkill(DRG.TrueThrust)]
    [ConflictingCombos(DRG_ST_SimpleMode, DRG_ST_AdvancedMode)]
    [CustomComboInfo("Gold Planner - Single Target (Option 3)",
        "Replaces True Thrust with the deterministic DRG GCD Planner (Hybrid Planner Option 3).\nGCD planning only for now; oGCD scoring and Decision Trace UI come next.",
        Job.DRG)]
    [AdvancedCombo]
    DRG_ST_GoldPlanner = 6050,
    ...
    [AutoAction(false, false)]
    [ReplaceSkill(DRG.DoomSpike)]
    [ConflictingCombos(DRG_AoE_SimpleMode, DRG_AoE_AdvancedMode)]
    [CustomComboInfo("Gold Planner - AoE (Option 3)",
        "Replaces Doom Spike with the deterministic DRG AoE GCD Planner (Hybrid Planner Option 3).\nPhase 5A: AoE variant of the Gold Planner for multi-target rotations.",
        Job.DRG)]
    [AdvancedCombo]
    DRG_AOE_GoldPlanner = 6250,
    ...
    [AutoAction(false, false)]
    [ReplaceSkill(SAM.Hakaze, SAM.Gyofu)]
    [ConflictingCombos(SAM_ST_SimpleMode, SAM_ST_AdvancedMode)]
    [CustomComboInfo("Gold Planner - Single Target (Phase 6)",
        "Replaces Hakaze with the deterministic SAM GCD Planner.\nPhase 6: Second job implementation to validate architecture scalability.",
        Job.SAM)]
    [AdvancedCombo]
    SAM_ST_GoldPlanner = 15150,

    [AutoAction(false, false)]
    [ReplaceSkill(SAM.Fuga, SAM.Fuko)]
    [ConflictingCombos(SAM_AoE_SimpleMode, SAM_AoE_AdvancedMode)]
    [CustomComboInfo("Gold Planner - AoE (Phase 7)",
        "Replaces Fuga with the deterministic SAM AoE GCD Planner.\nPhase 7: AoE variant completing SAM Gold Planner coverage.",
        Job.SAM)]
    [AdvancedCombo]
    SAM_AOE_GoldPlanner = 15151,
